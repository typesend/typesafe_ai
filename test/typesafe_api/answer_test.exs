defmodule TypeSafeAPI.AnswerTest do
  ...
  defp noul(value), do: %Noul{id: :x, noul: value, confidence: max(value, 1 - value)}

  defp choice_answer(confidence) do
    %Choice{
      id: :x,
      choice: :a,
      description: "A",
  ...
  describe "decode/4 — Score" do
    ...
    test "label comes from the question's levels, not the legend" do
      ...
      raw = %{
        ...
        "score" => 1.0,
        "legend" => %{
          "0" => %{"label" => "calm", "description" => "wording the API rephrased"},
          "1" => %{"label" => "upset", "description" => "also rephrased"}
        },
  ...
  describe "gate/2" do
    test "Noul uses max(noul, 1 - noul)" do
      assert Answer.gate(noul(0.9), act: 0.8, review: 0.55) == :act
      assert Answer.gate(noul(0.1), act: 0.8, review: 0.55) == :act
      assert Answer.gate(noul(0.6), act: 0.8, review: 0.55) == :review
      assert Answer.gate(noul(0.5), act: 0.8, review: 0.51) == :escalate
    end

    test "Choice uses confidence" do
      choice = choice_answer(0.82)
    ...
    test "raises when act is lower than review" do
      assert_raise ArgumentError, fn ->
        Answer.gate(noul(0.9), act: 0.4, review: 0.55)
      end
    end
  end
  ...
  describe "yes?/2" do
    test "defaults to a 0.5 threshold" do
      assert Answer.yes?(noul(0.51))
      refute Answer.yes?(noul(0.49))
      assert Answer.yes?(noul(0.5))
    end

    test "accepts an explicit threshold" do
      assert Answer.yes?(noul(0.7), 0.6)
      refute Answer.yes?(noul(0.5), 0.6)
    end

    test "a non-number threshold raises ArgumentError" do
      assert_raise ArgumentError, ~r/threshold/, fn -> Answer.yes?(noul(0.7), :high) end
    end
  end

  describe "confidence/1" do
    test "matches the value gate/2 uses for every answer type" do
      assert Answer.confidence(noul(0.9)) == 0.9
      assert Answer.confidence(noul(0.1)) == 0.9
      assert Answer.confidence(choice_answer(0.42)) == 0.42
    end
  end

  describe "decode/4 — Score strictness (hypothesis 1)" do
    test "integer probabilities are floated" do
      question = frustration_question()
      keys = keys_for(question, :frustration)
    ...
    test "a non-number probability is an unexpected error, not a wrong argmax" do
      question = frustration_question()
      keys = keys_for(question, :frustration)
      ...
      raw = %{
        "type" => "score",
        "score" => 0.0,
        "probabilities" => %{"0" => 1, "1" => "x", "2" => 0},
        "confidence" => 0.6
      }

      assert {:error, %Error{type: :unexpected, message: message}} =
               Answer.decode(raw, "frustration", question, keys)

      assert message =~ "probabilities"
    end

    test "empty probabilities is an error, not level 0" do
      question = frustration_question()
      keys = keys_for(question, :frustration)

      raw = %{"type" => "score", "score" => 2.0, "probabilities" => %{}, "confidence" => 0.9}
  ...
  describe "decode/4 — Score reconciliation (hypothesis 2)" do
    test "a probability key beyond the question's levels is an error" do
      question = frustration_question()
      keys = keys_for(question, :frustration)
      ...
      assert message =~ "3"
      assert message =~ "3 levels"
    end

    test "a legend that disagrees with the question's levels is an error naming both" do
      question =
        Question.Score.new("How angry?", [
          {"calm", "Calm and collected"},
          {"upset", "Visibly upset"}
      ...
      assert message =~ "calm"
      assert message =~ "totally different legend text"
    end

    test "a legend matching the question's levels decodes" do
      question = frustration_question()
      keys = keys_for(question, :frustration)
      ...
      assert {:ok, %Score{legend: legend}} = Answer.decode(raw, "frustration", question, keys)
      assert legend == %{0 => "Calm", 1 => "Frustrated", 2 => "Very angry"}
    end

    test "a structured legend entry matches the level it was sent as" do
      question = Question.Score.new("How angry?", [{"calm", "Calm"}, {"upset", "Visibly upset"}])
      keys = keys_for(question, :frustration)
      ...
      raw = %{
        ...
        "score" => 1.0,
        "legend" => %{
          "0" => %{"label" => "calm", "description" => "Calm"},
          "1" => %{"label" => "upset", "description" => "Visibly upset"}
        },
        "probabilities" => %{"0" => 0.1, "1" => 0.9},
        "confidence" => 0.9
      }

      assert {:ok, %Score{level: 1, label: "upset"}} =
               Answer.decode(raw, "frustration", question, keys)
    end
  end

  describe "decode/4 — Choice shape parity (hypothesis 3)" do
    test "missing options are completed with 0.0 and ordered like the question" do
      question = department_question()
      keys = keys_for(question)
      ...
      assert {:ok, %Choice{} = answer} = Answer.decode(raw, "department", question, keys)
      assert answer.probabilities == %{billing: 0.0, technical: 1.0, sales: 0.0}
      assert answer.options == [{:billing, 0.0}, {:technical, 1.0}, {:sales, 0.0}]
    end

    test "the chosen option's description is on the answer" do
      question = department_question()
      keys = keys_for(question)
      ...
      assert {:ok, %Choice{description: "Bugs, outages, integrations"}} =
               Answer.decode(raw, "department", question, keys)
    end

    test "an option the caller never declared is an unexpected error" do
      question = department_question()
      keys = keys_for(question)
      ...
      assert message =~ "legal"
    end

    test "a choice that is not the argmax is an unexpected error" do
      question = department_question()
      keys = keys_for(question)
      ...
      assert message =~ "technical"
    end

    test "empty probabilities is an error, not a 0.0 argmax the choice ties with" do
      question = department_question()
      keys = keys_for(question)
    ...
    test "a tie including the reported choice decodes" do
      question = department_question()
      keys = keys_for(question)
      ...
      raw = %{
        "type" => "choice",
        "choice" => "technical",
        "probabilities" => %{"billing" => 0.5, "technical" => 0.5, "sales" => 0.0},
        "confidence" => 0.5
      }

      assert {:ok, %Choice{choice: :technical}} = Answer.decode(raw, "department", question, keys)
    end
  end

  describe "decode/4 — Noul range (hypothesis 4)" do
    test "a value above 1.0 is an unexpected error, not a clamped value" do
      keys = Keys.build([{:is_urgent, Question.Noul.new("...")}])
      raw = %{"type" => "noul", "noul" => 1.4}
    ...
    test "a negative value is an unexpected error" do
      keys = Keys.build([{:is_urgent, Question.Noul.new("...")}])
      raw = %{"type" => "noul", "noul" => -0.2}

      assert {:error, %Error{type: :unexpected}} =
               Answer.decode(raw, "is_urgent", Question.Noul.new("..."), keys)
    end

    test "the bounds themselves decode, and integers are floated" do
      keys = Keys.build([{:is_urgent, Question.Noul.new("...")}])

      for value <- [0, 1, 0.0, 1.0] do
        assert {:ok, %Noul{noul: noul}} =
                 Answer.decode(
        ...
        assert is_float(noul)
      end
    end

    test "Noul carries a derived confidence field" do
      keys = Keys.build([{:is_urgent, Question.Noul.new("...")}])

      assert {:ok, %Noul{noul: 0.08, confidence: 0.92}} =
               Answer.decode(
                 %{"type" => "noul", "noul" => 0.08},
                 "is_urgent",
                 Question.Noul.new("..."),
  ...
  describe "yes?/2 and gate/2 on the wrong shape (hypothesis 5)" do
    test "yes?/2 on a Choice raises ArgumentError pointing at gate/2" do
      assert_raise ArgumentError, ~r/Choice.*gate\/2.*confidence\/1/s, fn ->
        Answer.yes?(choice_answer(0.9))
      end
    end

    test "gate/2 rejects a review threshold a Noul can never fall below" do
      assert_raise ArgumentError, ~r/0\.5/, fn ->
        Answer.gate(noul(0.9), act: 0.8, review: 0.5)
      end
    end
  end
end
